...
export class TaskService {
  ...
  async create(
    ...
    try {
      ...
      if (createTaskDto.memberIds && createTaskDto.memberIds.length > 0) {
        ...
        const validMemberIds = createTaskDto.memberIds.filter(
          id => projectMemberIds.includes(id) || id === project.ownerId
        );
  ...
  async findAll(
    ...
    if (assignedById) {
      queryBuilder.andWhere("task.assignedById = :assignedById", {
        assignedById,
      });
    }
  ...
  async update(
    ...
    try {
      ...
      if (updateTaskDto.memberIds) {
        ...
        const projectMemberIds = project.members.map(member => member.userId);
        const validMemberIds = updateTaskDto.memberIds.filter(
          id => projectMemberIds.includes(id) || id === project.ownerId
        );
